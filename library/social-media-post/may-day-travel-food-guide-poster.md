...
- 把 `{城市 / 景点}` 替换成真实目的地，并补充当地代表美食偏好。
- 可替换美食数量、卡片行列和节日文案，但建议保留 6 款结构。
- 如果城市美食特色很强，直接列出候选菜名会比让模型自动猜更准。

## 生成注意事项

- 美食图要检查食材是否符合当地习惯，避免“看着好吃但不地道”。
- 中文信息卡文字容易糊，正式发布建议后期重排文字。
- 如果画面拥挤，先删副标题、英文角标或减少每张卡片字数。
