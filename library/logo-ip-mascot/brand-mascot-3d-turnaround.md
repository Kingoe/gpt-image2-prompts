---
title: 3D 品牌吉祥物设定图
cover: ../../assets/covers/mascot-3d.svg
preview: ../../assets/previews/brand-mascot-3d-turnaround-generated.png
preview_type: generated
preview_source: generated from this prompt
scene: LOGO / IP 角色 / 吉祥物视觉
tags:
  - 品牌吉祥物
  - 3D 角色
  - 设定图
prompt: >
  Design a polished 3D brand mascot turnaround sheet, front and side character views,
  soft toy-like material, premium studio lighting, expressive but simple face, consistent
  proportions, collectible-character quality, suitable for brand identity decks, packaging,
  event visuals and merchandise development.
summary: 适合把吉祥物做成完整角色设定，方便后续延展包装、动画、周边和活动物料。
source: self-curated
collected_at: 2026-05-03
---

# 3D 品牌吉祥物设定图

## 效果预览

![3D 品牌吉祥物设定图预览](../../assets/previews/brand-mascot-3d-turnaround-generated.png)

## 适合什么时候用

适合品牌提案、角色设定、IP 延展、包装视觉、活动海报和周边开发。

## 提示词

```text
Design a polished 3D brand mascot turnaround sheet, front and side character views, soft toy-like material, premium studio lighting, expressive but simple face, consistent proportions, collectible-character quality, suitable for brand identity decks, packaging, event visuals and merchandise development.
```

## 使用建议

- 关键词 `front and side character views` 很适合做提案页和设定页。
- 如果想更偏手办感，可补 `vinyl toy finish`。
- 如果想更偏品牌主视觉，可弱化 turnaround，强化 pose 和 hero shot。

## 来源

- 原始来源：`self-curated`
- 收录时间：`2026-05-03`
